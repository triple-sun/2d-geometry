...
export class Polygon {
   ...
   */
  area() {
    let signedArea = [...this.faces].reduce(
    ...
    for (let edge of multiline) {
      ...
      for (let polygon of cutPolygons) {
        if (polygon.findEdgeByPoint(cut_edge_start) === undefined) {
          ...
          let [cutPoly1, cutPoly2] = polygon.cutFace(
            cut_edge_start,
            cut_edge_end
          );
  ...
  cutWithLine(line) {
    ...
    intersections.int_points1_sorted = getSortedArrayOnLine(
      line,
      intersections.int_points1
    );
    intersections.int_points2_sorted = getSortedArray(
    ...
    intersections.int_points1 = intersections.int_points1.filter(
      (int_point) => int_point.id >= 0
    );
    intersections.int_points2 = intersections.int_points2.filter(
      (int_point) => int_point.id >= 0
    );
    ...
    for (let int_point1_curr of intersections.int_points1_sorted) {
      if (int_point1_curr.edge_before.bv === INSIDE) {
        new_edge = new Flatten.Edge(
          new Flatten.Segment(int_point1_prev.pt, int_point1_curr.pt)
        ); // (int_point1_curr.edge_before.shape);
        ...
        new_edge = new Flatten.Edge(
          new Flatten.Segment(int_point1_curr.pt, int_point1_prev.pt)
        ); // (int_point1_curr.edge_before.shape.reverse());
        insertBetweenIntPoints(
          intersections.int_points2[int_point1_curr.id],
          intersections.int_points2[int_point1_prev.id],
          new_edge
  ...
  splitToIslands() {
    ...
    let newPolygons = polygons.filter(
      (polygon) => [...polygon.faces][0].orientation() === orientation
    );
  ...
  distanceTo(shape) {
    ...
    if (shape instanceof Flatten.Point) {
      let [dist, shortest_segment] = Flatten.Distance.point2polygon(
        shape,
        this
      );
    ...
    if (
      ...
      let [dist, shortest_segment] = Flatten.Distance.shape2polygon(
        shape,
        this
      );
      shortest_segment = shortest_segment.reverse();
    ...
    if (shape instanceof Flatten.Polygon) {
      ...
      for (let edge of this.edges) {
        ...
        [dist, shortest_segment] = Flatten.Distance.shape2planarSet(
          edge.shape,
          shape.edges,
          min_stop
        );
  ...
  rotate(angle = 0, center = new Flatten.Point()) {
    ...
    for (let face of this.faces) {
      newPolygon.addFace(
        face.shapes.map((shape) => shape.rotate(angle, center))
      );
    }
    return newPolygon;
  }
